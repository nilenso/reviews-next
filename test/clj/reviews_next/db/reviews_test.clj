...
(deftest insert-test
  (testing "Test insert to reviews table"
    (let [conn (config/connection-uri)]
      (is (= false (reviews/insert {:title "Report"} conn)))
      (is (= false (reviews/insert {:review_date ""} conn)))
      (is (= false (reviews/insert {:title "" :review_date ""} conn)))
      (is (= false (reviews/insert {:title "" :review_date "20-09-2017"} conn)))
      (is (= false (reviews/insert {:title "Report" :review_date ""} conn)))

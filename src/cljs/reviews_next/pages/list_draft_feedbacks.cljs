...
(defn feedbacks-table-component [logged-in-user-details]
  (reagent/with-let [draft-feedback-list (re-frame/subscribe [::subs/draft-feedbacks-list])]
        ...
        [:div
         ...
         [:div.feedback-table (use-style table-style)
          [:div.head-row (use-style head-row-style)
           ...
           [:div {:style {:width "20%"}}]]
          (js/console.log "feedback render again!" (count @draft-feedback-list))
          (for [feedback @draft-feedback-list]
            ...
            [:div.row (use-style row-style)
             ...
             [components/Button {:onClick #((re-frame/dispatch [::events/publish-draft-feedback (:id feedback)])
                                            (js/console.log "ID: " (:id logged-in-user-details))
                                            (re-frame/dispatch [::events/populate-draft-feedback-list (:id logged-in-user-details)]))
                                 :width "20%"
  ...
  (let [logged-in-user-details {:id "U1" :name "ABC"}]
    [:div.main-content (use-style main-content-style)
     ...
     [:div.main-section (use-style (section-style "80vw"))
      ...
      [components/Button {:onClick #(accountant/navigate! "/list-feedbacks")
                          :width "20%"
                          :style {:margin "0px 5px"
                                  :color "#00947E"}} "Published Ones"]]]))
